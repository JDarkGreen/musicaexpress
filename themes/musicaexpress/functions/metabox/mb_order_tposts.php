...
function orderAllItems()
{
	if( getPublishTypePosts( getTypePost() ) ):
        ...
        update_post_meta( $tpost->ID , 'mbox_order_tpost' , $control );
...
function sortMetaboxItems( $item_order1 , $item_order2 )
{
	if( getPublishTypePosts( getTypePost() ) ):
		 ...
		 */
		update_post_meta( $element_from , 'mbox_order_tpost' , $item_order2 );
		update_post_meta( $element_to , 'mbox_order_tpost' ,  $item_order1 );
...
{
	$mb_order = get_post_meta( $postID , 'mbox_order_tpost' , true );
...
function cd_mb_order_type_posts_save( $post_id )
{
    ...
    if( isset( $_POST['select_order_tpost_mbox'] ) && !empty( $_POST['select_order_tpost_mbox'] ) ) :
    	...
    	if( isset( $_POST['mb_current_order_mbox'] ) )
			add_post_meta( $post_id , 'mbox_order_tpost', intval($_POST['mb_current_order_mbox']) );
